...
export function Stat({ label, value, sub, icon, tone = "blue", money = false }: { label: string; value: number; sub?: ReactNode; icon: string; tone?: "blue" | "gold" | "green" | "red" | "navy"; money?: string | false }) {
  const tt = useT();
  ...
  return (
    <div className="panel p-4 flex items-start gap-3.5 hover:shadow-lift hover:-translate-y-0.5 transition-all duration-200 group">
      ...
      <div className="min-w-0">
        ...
        <div className="font-display text-[22px] leading-7 font-bold tnum truncate">{money ? `${Math.round(v).toLocaleString("en-US")} ${money}` : Math.round(v).toLocaleString("en-US")}</div>
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center p-0 sm:p-6" role="dialog" aria-modal="true">
      ...
      <div className={`relative w-full ${w} panel pop-in rounded-b-none sm:rounded-xl max-h-[92vh] flex flex-col shadow-pop`}>
        ...
        {footer && <div className="px-5 py-3.5 border-t border-ink-100 dark:border-ink-800 flex flex-wrap justify-end gap-2 bg-ink-50/50 dark:bg-ink-950/40 rounded-b-xl">{footer}</div>}
      </div>
    </div>
  );
}
    ...
    <div className="flex items-center gap-4">
      <div className="relative shrink-0" style={{ width: size, height: size }}>
        <svg viewBox="0 0 100 100" width={size} height={size} className="-rotate-90">
          ...
          {segments.map((s, i) => {
            const frac = s.value / total; const len = frac * C; const off = acc; acc += len;
            return <circle key={i} cx="50" cy="50" r={r} fill="none" stroke={s.color} strokeWidth="11"
              strokeDasharray={`${Math.max(0, len - 2)} ${C}`} strokeDashoffset={-off} className="donut-anim" style={{ transition: "stroke-dasharray .8s" }} />;
          })}
        </svg>
...
export function PrintPortal({ children }: { children: ReactNode }) {
  const el = typeof document !== "undefined" ? document.getElementById("print-sheet") : null;
  if (!el) return null;
...
export function Toaster() {
  ...
  return (
    <div className="fixed bottom-20 lg:bottom-4 right-4 z-[100] flex flex-col gap-2 pointer-events-none" role="status" aria-live="polite">
      {list.map((t) => (
