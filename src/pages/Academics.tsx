...
import { Modal, Field, Chip, Avatar, toast, printNow } from "../components/ui";
...
export function SubjectsPage() {
  ...
  const save = () => {
    ...
    toast("Subject created"); setForm(false); setF({ name: "", code: "", credits: 2, levels: "4,5,6" });
  };
  return (
    <div>
      <PageHead title="Subjects" sub={`${db.subjects.length} subjects in the ${db.school.academicYear} curriculum`}>
        <button className="btn-p btn-sm" onClick={() => setForm(true)}><Ic n="plus" size={15} />New subject</button>
      </PageHead>
              ...
              {db.subjects.map((x) => {
                ...
                return (
                  <tr key={x.id}>
                    ...
                    <td>{lead ? <span className="flex items-center gap-2"><Avatar first={lead.first} last={lead.last} hue={lead.hue} size={26} /><span className="text-[13px] font-semibold whitespace-nowrap">{lead.first} {lead.last}</span></span> : <Chip tone="amber">Unassigned</Chip>}</td>
                    <td>
                      <select className="input !h-8 !text-[12px] !w-auto" defaultValue="" aria-label={`Assign teacher to ${x.name}`}
                        onChange={(e) => { if (!e.target.value) return; mutate((db) => { const sub = db.subjects.find((y) => y.id === x.id)!; if (!sub.teacherIds.includes(e.target.value)) sub.teacherIds.push(e.target.value); }); audit("ASSIGN_TEACHER", "Subject", `${db.teachers.find((t) => t.id === e.target.value)?.first} → ${x.name}`); toast("Teacher assigned"); e.target.value = ""; }}>
      ...
      <Modal open={form} onClose={() => setForm(false)} title="New subject" w="max-w-md"
        footer={<><button className="btn-o" onClick={() => setForm(false)}>Cancel</button><button className="btn-p" onClick={save}><Ic n="check" size={15} />Create subject</button></>}>
        <div className="grid grid-cols-2 gap-4">
...
export function TimetablePage() {
  ...
  const title = mode === "class" ? `${db.classes.find((c) => c.id === cls)?.name} ${db.classes.find((c) => c.id === cls)?.section}` : `${db.teachers.find((t) => t.id === tch)?.first} ${db.teachers.find((t) => t.id === tch)?.last}`;

  return (
    <div>
      <PageHead title="Timetable" sub="Class and teacher schedules — conflicts detected automatically.">
        <button className="btn-o btn-sm" onClick={() => {
          printNow(
            <div className="p-2 max-w-[820px] mx-auto">
              <div className="flex items-end justify-between border-b-4 border-ink-950 pb-3 mb-4">
                <div>
                  ...
                  <div className="text-[11px] uppercase tracking-[0.18em] text-ink-500">{mode === "class" ? "Class timetable" : "Teacher timetable"} — {title}</div>
                </div>
                <div className="text-right text-[11px] text-ink-500">{db.school.academicYear} · {db.school.term}</div>
              </div>
              <table className="w-full border-collapse text-[11.5px]">
                <thead><tr><th className="border border-ink-300 bg-ink-950 text-white px-2 py-1.5 text-left">Time</th>{DAYS.map((d) => <th key={d} className="border border-ink-300 bg-ink-950 text-white px-2 py-1.5">{d}</th>)}</tr></thead>
                <tbody>
                  {PERIODS.map((per, p) => (
                    <tr key={p}>
                      <td className="border border-ink-300 px-2 py-2 font-bold whitespace-nowrap">{per[0]}–{per[1]}</td>
                      {DAYS.map((_, day) => {
                        const c = cell(day, p);
                        return <td key={day} className="border border-ink-300 px-2 py-2 align-top">
                          {c ? <><b className="block">{c.sub?.name}</b><span className="text-ink-500">{mode === "class" ? db.teachers.find((t) => t.id === c.slot.teacherId)?.last : (() => { const k = db.classes.find((x) => x.id === c.slot.classId); return k ? `${k.name} ${k.section}` : ""; })()}</span><span className="block text-[9.5px] text-ink-400">{c.slot.room}</span></> : <span className="text-ink-200">—</span>}
                        </td>;
                      })}
                    </tr>
                  ))}
              ...
              <p className="text-[10px] text-ink-400 mt-3">Generated by VITECH School Management System · {db.school.name}</p>
            </div>
          );
        }}><Ic n="printer" size={15} />Print</button>
      </PageHead>
      <div className="flex flex-wrap gap-2.5 mb-4">
        ...
        {conflicts.length > 0 ? <Chip tone="red" className="!py-2"><Ic n="alert" size={13} />{conflicts.length} teacher conflicts</Chip> : <Chip tone="green" className="!py-2"><Ic n="check" size={13} />No conflicts</Chip>}
      </div>
      ...
      {conflicts.length > 0 && (
        <div className="panel mt-4 p-4 !border-amber-300 dark:!border-amber-700">
          ...
          <button className="btn-o btn-sm mt-3" onClick={() => {
            const rows = conflicts.map((c) => { const t = db.teachers.find((x) => x.id === c.teacherId); return [t ? `${t.first} ${t.last}` : c.teacherId, DAYS[c.day], c.start, `"${c.classes.map((cid) => { const k = db.classes.find((x) => x.id === cid); return `${k?.name} ${k?.section}`; }).join(" vs ")}"`].join(","); });
            const blob = new Blob([["Teacher,Day,Start,Classes", ...rows].join("\n")], { type: "text/csv" });
            const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "timetable-conflicts.csv"; a.click();
            setTimeout(() => URL.revokeObjectURL(a.href), 3000);
            audit("EXPORT_TIMETABLE", "Timetable", "Conflict report exported");
            toast("Conflict report exported");
          }}><Ic n="download" size={14} />Export conflict report</button>
        </div>
      )}
