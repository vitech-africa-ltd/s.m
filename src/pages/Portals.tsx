    ...
    <div>
      <div className="panel !rounded-2xl overflow-hidden mb-4">
        <div className="bg-ink-950 text-ink-100 px-6 py-6 flex flex-wrap items-center gap-5">
          ...
          <div className="flex-1 min-w-[200px]">
            <div className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-gold-400">{tt("Student portal")}</div>
          ...
          <div className="flex gap-3">
            <div className="rounded-xl bg-white/[0.07] border border-white/10 px-5 py-3 text-center"><div className="font-display font-bold text-[22px] tnum" style={{ color: avg >= db.school.passMark ? "#34d399" : "#fb7185" }}>{avg ? gl.grade : "—"}</div><div className="text-[10px] font-extrabold uppercase tracking-wide text-ink-400">{tt("Term grade")}</div></div>
            <div className="rounded-xl bg-white/[0.07] border border-white/10 px-5 py-3 text-center"><div className="font-display font-bold text-[22px] tnum">{avg ? avg.toFixed(1) : "—"}</div><div className="text-[10px] font-extrabold uppercase tracking-wide text-ink-400">{tt("Average %")}</div></div>
          </div>
        </div>
      </div>
      <div className="grid lg:grid-cols-3 gap-4 mb-4">
        <div className="panel p-5 flex items-center gap-5">
          ...
          <div><h3 className="font-display font-bold text-[16px]">{tt("My attendance")}</h3><p className="text-[12.5px] text-ink-400 mt-1">{tt("Last 15 school days")}. {tt("Class average today")}: <b className="text-ink-700 dark:text-ink-100">{attPct(db, todayISO(), st.classId)}%</b>.</p></div>
        </div>
        <div className="panel p-5">
          <h3 className="font-display font-bold text-[16px] mb-3">{tt("Fees status")}</h3>
          <div className="flex justify-between text-[13px] font-semibold"><span>{tt("Annual fees")}</span><b className="tnum">{fmtMoney(total, cur)}</b></div>
          <div className="flex justify-between text-[13px] font-semibold mt-1"><span>{tt("Paid")}</span><b className="tnum text-emerald-600">{fmtMoney(paid, cur)}</b></div>
          ...
          <p className={`text-[12.5px] font-bold mt-2 ${total - paid > 0 ? "text-rose-600" : "text-emerald-600"}`}>{total - paid > 0 ? `${tt("Balance")}: ${fmtMoney(total - paid, cur)}` : tt("Fully paid — well done!")}</p>
        </div>
        <div className="panel p-5">
          <h3 className="font-display font-bold text-[16px] mb-3">{tt("Next exam")}</h3>
          {db.exams.filter((e) => e.status === "scheduled").map((e) => (
            <div key={e.id} className="rounded-lg border border-ink-100 dark:border-ink-800 px-4 py-3">
              ...
              <Chip tone="amber" className="mt-2"><Ic n="clock" size={11} />{Math.max(0, Math.round((new Date(e.date).getTime() - Date.now()) / 864e5))} {tt("days away")}</Chip>
            </div>
          ))}
        </div>
      </div>
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="panel overflow-hidden">
          <div className="panel-h"><h3 className="font-display font-bold text-[16px]">{tt("My results")} — {exam.name}</h3><button className="btn-o btn-sm" onClick={() => nav("/app/reportcards")}><Ic n="award" size={14} />{tt("Report card")}</button></div>
          <div className="overflow-x-auto">
            <table className="tbl">
              <thead><tr><th>{tt("Subject")}</th><th className="!text-center">{tt("Mark")}</th><th>{tt("Grade")}</th></tr></thead>
              <tbody>
                {grades.map((g) => {
        ...
        <div className="panel overflow-hidden">
          <div className="panel-h"><h3 className="font-display font-bold text-[16px]">{tt("My timetable")}</h3><Chip tone="blue">{tt("Room")} {c?.room}</Chip></div>
          <div className="overflow-x-auto">
            <table className="tbl !text-[12px]">
              <thead><tr><th>{tt("Time")}</th>{DAYS.map((d) => <th key={d} className="!text-center">{d.slice(0, 3)}</th>)}</tr></thead>
              <tbody>
      ...
      <div className="panel p-5 mt-4">
        <h3 className="font-display font-bold text-[16px] mb-3">{tt("Announcements")}</h3>
...
export function ParentPortal() {
  ...
  if (!st) return <Empty icon="students" title={tt("No children linked to this account")} />;
  const c = classOf(db, st);
  const days = lastSchoolDays(15);
  const att = Math.round((days.filter((d) => ["P", "L"].includes(attStatus(db, d, st.id))).length / days.length) * 100);
  const exam = db.exams.find((e) => e.status === "completed")!;
  ...
  return (
    <div>
      <PageHead title={tt("Parent portal")} sub={tt("Your children's progress, attendance and fees at a glance.")}>
        {kids.length > 1 && (
      ...
      <div className="panel !rounded-2xl p-6 mb-4 flex flex-wrap items-center gap-5">
        ...
        <div className="flex-1 min-w-[180px]">
          ...
          <p className="text-[13px] text-ink-400 font-semibold">{st.regNo} · {c?.name} {c?.section} · {tt("Class teacher")}: {teacher?.first} {teacher?.last}</p>
        </div>
        <div className="flex gap-3">
          {([[tt("Average"), avg ? `${avg.toFixed(1)}%` : "—"], [tt("Grade"), avg ? gl.grade : "—"], [tt("Attendance"), `${att}%`]] as [string, string][]).map(([k, v]) => (
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="panel p-5">
          <h3 className="font-display font-bold text-[16px] mb-3">{tt("Fees & balance")}</h3>
          <div className="flex justify-between text-[13px] font-semibold"><span>{tt("Annual fees")}</span><b className="tnum">{fmtMoney(total, cur)}</b></div>
          <div className="flex justify-between text-[13px] font-semibold mt-1"><span>{tt("Paid")}</span><b className="tnum text-emerald-600">{fmtMoney(paid, cur)}</b></div>
          <div className="h-2.5 rounded-full bg-ink-100 dark:bg-ink-800 overflow-hidden mt-3"><div className={`h-full barx-anim ${total - paid > 0 ? "bg-gold-400" : "bg-emerald-500"}`} style={{ width: `${Math.min(100, (paid / Math.max(1, total)) * 100)}%` }} /></div>
          <p className={`text-[12.5px] font-bold mt-2 ${total - paid > 0 ? "text-rose-600" : "text-emerald-600"}`}>{total - paid > 0 ? `${tt("Please clear")} ${fmtMoney(total - paid, cur)}` : tt("No balance — thank you!")}</p>
        ...
        <div className="panel p-5">
          <h3 className="font-display font-bold text-[16px] mb-3">{tt("Latest results")}</h3>
        ...
        <div className="panel p-5">
          <h3 className="font-display font-bold text-[16px] mb-3">{tt("School announcements")}</h3>
          ...
          <div className="rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-800 px-4 py-3 mt-3 text-[12px] font-semibold text-emerald-700 dark:text-emerald-300 flex gap-2">
            <Ic n="whatsapp" size={15} className="shrink-0 mt-0.5" />{tt("You receive absence alerts, payment confirmations and exam reminders on WhatsApp & SMS.")}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <div className="panel !rounded-2xl bg-ink-950 !text-ink-100 !border-ink-800 p-6 mb-4 flex flex-wrap items-center gap-5">
        ...
        <div className="flex-1 min-w-[200px]">
          <div className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-gold-400">{tt("Teacher portal")}</div>
          <h1 className="font-display text-[24px] font-bold">{t.first} {t.last}</h1>
          <p className="text-[13px] text-ink-300 font-semibold">{t.specialization} · {t.empNo} · {tt("You can access only your assigned classes.")}</p>
        </div>
        <div className="flex gap-2.5">
          <button className="btn-gold" onClick={() => nav("/app/attendance")}><Ic n="attendance" size={15} />{tt("Take attendance")}</button>
          <button className="btn-o !bg-white/[0.07] !border-white/15 !text-white" onClick={() => nav("/app/grades")}><Ic n="grades" size={15} />{tt("Enter grades")}</button>
        </div>
      </div>
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3.5 mb-4">
        {([[tt("My classes"), myClasses.length, "class"], [tt("My students"), myStudents.length, "students"], [tt("Subjects"), mySubjects.length, "subject"], [tt("Periods / week"), mySlots.length, "timetable"]] as [string, number, string][]).map(([l, v, ic]) => (
          <div key={l} className="panel p-4 flex items-center gap-3.5"><span className="w-10 h-10 rounded-lg bg-cobalt-50 dark:bg-cobalt-500/15 text-cobalt-600 dark:text-cobalt-300 flex items-center justify-center"><Ic n={ic} /></span><div><div className="text-[11px] font-extrabold uppercase tracking-[0.1em] text-ink-400">{l}</div><div className="font-display text-[20px] font-bold tnum">{v}</div></div></div>
        ))}
      </div>
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="panel p-5">
          <h3 className="font-display font-bold text-[16px] mb-3">{tt("My classes")}</h3>
          <div className="space-y-2.5">
            {myClasses.slice(0, 6).map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => nav("/app/attendance")} className="w-full flex items-center gap-3.5 rounded-lg border border-ink-100 dark:border-ink-800 px-4 py-3 hover:border-cobalt-400 transition-colors text-left cursor-pointer">
                  ...
                  <span className="flex-1"><b className="block text-[13.5px]">{c.name} — {tt("Section")} {c.section}</b><span className="text-[11.5px] text-ink-400 font-semibold">{tt("Room")} {c.room} · {n} {tt("students")}</span></span>
        ...
        <div className="panel p-5">
          <h3 className="font-display font-bold text-[16px] mb-3">{tt("Today's schedule")}</h3>
          <div className="space-y-2">
            ...
            {mySlots.filter((x) => x.day === (new Date().getDay() + 6) % 7).length === 0 && <p className="text-[13px] text-ink-300 font-semibold py-4 text-center">{tt("No periods today — enjoy the free day.")}</p>}
          </div>
          <h3 className="font-display font-bold text-[16px] mt-5 mb-2">{tt("Announcements for staff")}</h3>
