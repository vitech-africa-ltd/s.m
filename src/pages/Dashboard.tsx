...
export default function Dashboard({ nav }: { nav: (to: string) => void }) {
  ...
  const d = useMemo(() => {
    ...
    const byClass = db.classes.map((c) => ({ label: `${c.name.slice(0, 2)}${c.level} ${c.section}`, value: Math.round(db.students.filter((x) => x.classId === c.id).reduce((a, x) => a + Math.min(paidBy(db, x.id), feeTotal(db, c.level)), 0) / 1000) })).sort((a, b) => b.value - a.value).slice(0, 6);
  ...
  const collectionPct = Math.min(100, Math.round((db.payments.reduce((a, b) => a + b.amount, 0) / Math.max(1, db.students.filter((x) => x.status === "active").reduce((a, x) => a + feeTotal(db, classOf(db, x)?.level ?? 1), 0))) * 100));
  ...
  return (
    <div>
      ...
      <div className="relative overflow-hidden panel !rounded-2xl p-6 mb-5 bg-ink-950 !text-ink-100 !border-ink-800">
        ...
        <div className="absolute -right-16 -top-16 w-64 h-64 rounded-full bg-cobalt-600/20 blur-3xl" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3.5 mb-5">
        ...
        <Stat label="Attendance today" value={d.att} sub={`${d.statuses.A} ${tt("absent")} · ${d.statuses.L} ${tt("late")}`} icon="attendance" tone="green" />
        ...
        <Stat label="Monthly revenue" value={d.mRev} money={cur} sub={`▲ vs ${fmtMoney(d.revBy[d.revBy.length - 2], cur)}`} icon="coins" />
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-4">
        ...
        <Reveal delay={80}>
          <div className="panel h-full">
            <div className="panel-h"><h3 className="font-display font-bold text-[16px]">{tt("Attendance today")}</h3><Chip tone="blue">{fmtNum(d.active)} {tt("students")}</Chip></div>
          ...
          <div className="panel h-full">
            ...
            <div className="px-5 pb-5">
              <div className="flex items-center gap-4 mb-4">
                <Ring value={collectionPct} size={92} color="#c98f1b" />
              ...
              <HBars rows={d.byClass.map((c, i) => ({ ...c, color: ["#1e49c9", "#2b5ce9", "#4f7df3", "#dca638", "#c98f1b", "#6f90c2"][i] }))} money="K" />
            </div>
          </div>
        </Reveal>
        <Reveal delay={140}>
                  ...
                  {[...db.payments].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 7).map((p) => {
                    ...
                    return (
                      <tr key={p.id}>
                        ...
                        <td><span className="flex items-center gap-2.5">{st && <Avatar first={st.first} last={st.last} hue={st.hue} size={28} />}<span><span className="block font-bold text-[13px] whitespace-nowrap">{st?.first} {st?.last}</span><span className="block text-[11px] text-ink-400">{st?.regNo}</span></span></span></td>
