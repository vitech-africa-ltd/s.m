...
import { useApp, mutate, audit, notify, uid, todayISO, fmtDate, fmtMoney, fmtNum, monthKeys, monthLabel, attPct, collectionRate, CURRENCY_MAP, type Doc, type Plan } from "../lib/data";
import { Ic } from "../components/icons";
import { Modal, Field, Chip, Avatar, Empty, toast, AreaChart, Donut, HBars, Ring, useCountUp } from "../components/ui";
...
export function LibraryPage() {
  ...
  const doBorrow = () => {
    ...
    mutate((db) => {
      ...
      db.loans.unshift({ id: uid(), bookId: borrow, borrower: st ? `${st.first} ${st.last}` : "Guest", type: "student", date: todayISO(), due: new Date(Date.now() + 14 * 864e5).toISOString().slice(0, 10), returned: null, fine: 0 });
    });
    ...
    <div>
      <PageHead title="Library" sub={`${db.books.reduce((a, b) => a + b.copies, 0)} copies · ${activeLoans.length} on loan · ${activeLoans.filter((l) => l.due < todayISO()).length} overdue`}>
        <button className="btn-o btn-sm" onClick={() => {
          const blob = new Blob([["Title,Author,Category,ISBN,Copies,Available", ...db.books.map((b) => [`"${b.title}"`, `"${b.author}"`, b.category, b.isbn, b.copies, b.available].join(","))].join("\n")], { type: "text/csv" });
          const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "library-catalogue.csv"; a.click();
          setTimeout(() => URL.revokeObjectURL(a.href), 3000);
          audit("EXPORT_LIBRARY", "Library", "Catalogue exported"); toast("Catalogue exported");
        }}><Ic n="download" size={15} />Catalogue</button>
      </PageHead>
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-4">
        <div className="panel overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="tbl">
              <thead><tr><th>Title</th><th>Category</th><th className="hidden md:table-cell">ISBN</th><th>Copies</th><th className="!text-right">Action</th></tr></thead>
              <tbody>
                {books.map((b) => (
                  <tr key={b.id}>
                    ...
                    <td className="hidden md:table-cell font-mono text-[11.5px] text-ink-400">{b.isbn}</td>
          ...
          {db.vehicles.map((v) => {
            ...
            return (
              <div key={v.id} className="panel p-4 flex items-center gap-4 hover:shadow-lift transition-shadow">
                ...
                <button className="btn-g btn-sm" onClick={() => { mutate((db) => { const x = db.vehicles.find((y) => y.id === v.id)!; x.status = x.status === "active" ? "maintenance" : "active"; }); toast(`${v.plate} status updated`, "info"); }}><Ic n="settings" size={14} /></button>
              </div>
            );
          })}
        </div>
        <div className="space-y-3.5">
          ...
          {db.routes.map((r) => (
            <div key={r.id} className="panel p-4">
              ...
              <div className="flex items-center justify-between mt-3 text-[12px] font-semibold text-ink-400"><span>{r.students} students subscribed</span>
                <button className="btn-g btn-sm" onClick={() => {
                  const blob = new Blob([[`Route: ${r.name}`, `Fee: ${fmtMoney(r.fee, db.school.currency)}/term`, `Stops: ${r.stops.join(" → ")}`, `Students: ${r.students}`].join("\n")], { type: "text/plain" });
                  const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `${r.name.replace(/\s+/g, "-")}-manifest.txt`; a.click();
                  setTimeout(() => URL.revokeObjectURL(a.href), 3000);
                  audit("EXPORT_MANIFEST", "Transport", r.name); toast("Route manifest exported");
                }}><Ic n="download" size={13} />Manifest</button></div>
            </div>
          ))}
        </div>
      </div>
    ...
    <div>
      ...
      {tab === "staff" ? (
        <div className="panel overflow-hidden">
          <div className="overflow-x-auto">
            <table className="tbl">
              <thead><tr><th>Employee</th><th>Department</th><th>Position</th><th className="hidden sm:table-cell">Hired</th><th>Salary</th><th>Status</th></tr></thead>
              <tbody>
                {db.staff.map((x) => (
                  <tr key={x.id}>
                    ...
                    <td className="hidden sm:table-cell text-[12.5px] text-ink-400 whitespace-nowrap">{fmtDate(x.hired)}</td>
...
export function DocumentsPage() {
  ...
  const kindIc: Record<string, string> = { pdf: "file", img: "eye", xls: "grades", doc: "exams" };
  const download = (d: Doc) => {
    const body = [`VITECH SCHOOL MANAGEMENT SYSTEM — DOCUMENT`, `Name: ${d.name}`, `Category: ${d.category}`, `Size: ${d.size}`, `Uploaded: ${d.date} by ${d.by}`, ``, `Secured document export — access controlled by role-based permissions.`, `School: ${db.school.name} · ${db.school.academicYear}`].join("\n");
    const blob = new Blob([body], { type: "text/plain" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = d.name.replace(/\s+/g, "_") + ".txt"; a.click();
    setTimeout(() => URL.revokeObjectURL(a.href), 3000);
    audit("DOWNLOAD_DOCUMENT", "Documents", d.name); toast(`Downloading ${d.name}`);
  };
  return (
    <div>
            ...
            {docs.map((d) => (
              <div key={d.id} className="rounded-xl border border-ink-100 dark:border-ink-800 p-4 hover:border-cobalt-300 dark:hover:border-cobalt-700 hover:shadow-panel transition-all group">
                ...
                <div className="flex gap-1.5 mt-3">
                  <button className="btn-g btn-sm flex-1" onClick={() => setPreview(d)}><Ic n="eye" size={13} />View</button>
                  <button className="btn-g btn-sm flex-1" onClick={() => download(d)}><Ic n="download" size={13} />Get</button>
      ...
      <Modal open={!!preview} onClose={() => setPreview(null)} title={preview?.name ?? "Document"} w="max-w-lg">
        {preview && (
          <div>
            <div className="rounded-xl border border-ink-100 dark:border-ink-800 bg-ink-50 dark:bg-ink-950/60 flex flex-col items-center justify-center py-10 gap-3">
              <span className="w-14 h-14 rounded-xl bg-rose-50 dark:bg-rose-500/10 text-rose-500 flex items-center justify-center"><Ic n="file" size={26} /></span>
              <div className="text-[13px] font-bold">{preview.name}</div>
              <Chip tone="gray">{preview.kind.toUpperCase()} · {preview.size}</Chip>
            </div>
            <div className="grid grid-cols-2 gap-3 mt-4 text-[12.5px]">
              <div><div className="text-[10px] font-extrabold uppercase tracking-wide text-ink-400">Category</div><div className="font-semibold">{preview.category}</div></div>
              <div><div className="text-[10px] font-extrabold uppercase tracking-wide text-ink-400">Uploaded</div><div className="font-semibold">{fmtDate(preview.date)} · {preview.by}</div></div>
            </div>
            <div className="flex gap-2 mt-5">
              <button className="btn-p btn-sm flex-1" onClick={() => download(preview)}><Ic n="download" size={14} />Download</button>
              <button className="btn-o btn-sm flex-1" onClick={() => setPreview(null)}>Close</button>
            </div>
          </div>
        )}
      </Modal>
...
export function CertificatesPage() {
  ...
  return (
    <div>
      ...
      <div className="flex justify-end mb-4"><button className="btn-p btn-sm" onClick={() => setIssue(true)}><Ic n="plus" size={15} />Issue certificate</button></div>
        ...
        {db.certificates.map((c) => (
          <div key={c.id} className={`panel p-5 relative overflow-hidden ${!c.valid ? "opacity-60" : ""}`}>
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-dashed border-ink-200 dark:border-ink-700">
              <div className="flex items-center gap-2.5"><QR value={`https://${db.school.website}/verify?q=${c.code}`} size={40} /><span className="font-mono text-[12px] font-bold text-cobalt-600 dark:text-cobalt-300">{c.code}</span></div>
              <span className="text-[11.5px] text-ink-400 font-semibold">{fmtDate(c.date)}</span>
            </div>
            <button className="btn-g btn-sm w-full mt-3" onClick={() => { mutate((db) => { const x = db.certificates.find((y) => y.id === c.id)!; x.valid = !x.valid; }); audit("REVOKE_CERTIFICATE", "Certificate", c.code); toast(c.valid ? "Certificate revoked" : "Certificate restored", "info"); }}><Ic n={c.valid ? "x" : "refresh"} size={13} />{c.valid ? "Revoke" : "Restore"}</button>
          </div>
        ))}
      </div>
...
export function VerifyPage({ nav }: { nav: (to: string) => void }) {
  const s = useApp();
  const [code, setCode] = useState(new URLSearchParams(window.location.hash.split("?")[1] ?? "").get("q") ?? "");
  ...
  const check = () => setResult(!code.trim() ? null : !found ? "none" : found.valid ? "ok" : "bad");
  return (
    ...
    <div>
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {list.map((p) => {
          ...
          const code = isSt ? (p as { regNo: string }).regNo : (p as { empNo: string }).empNo;
          return (
            <div key={p.id} className="print-card rounded-xl overflow-hidden border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 shadow-panel hover:shadow-lift hover:-translate-y-0.5 transition-all">
              <div className="bg-ink-950 text-white px-4 py-2.5 flex items-center gap-2">
                <span className="w-6 h-6 rounded bg-gold-400 text-ink-950 font-display font-bold flex items-center justify-center text-[13px]">{(db.school.logoText || "V")[0]}</span>
              ...
              <div className="p-4 flex items-center gap-3.5">
                {isSt ? <Avatar first={p.first} last={p.last} hue={p.hue} size={56} photo={`https://image.qwenlm.ai/generated-images/${["25138a51-2482-4c24-99d4-fd9f04a8f68e", "364b1141-245a-454b-9fee-89476d717481", "db882df6-fa78-47b1-9ea7-605f2ab456c1", "c09317da-c904-4596-81c5-2974bd7e9509"][(p.first.length + p.id.length) % 4]}/_result.png`} /> : <Avatar first={p.first} last={p.last} hue={p.hue} size={56} />}
                <div className="min-w-0 flex-1">
                  <b className="block font-display text-[14.5px] leading-tight truncate">{p.first} {p.last}</b>
                  <span className="text-[10.5px] font-mono font-bold text-cobalt-600 dark:text-cobalt-300">{code}</span>
                ...
                <QR value={`VITECH|${code}|${db.school.website}`} size={40} />
              </div>
  ...
  return (
    <div>
      <PageHead title="Audit logs" sub="Every significant action — who, what, when, from where.">
        <button className="btn-o btn-sm" onClick={() => { const blob = new Blob([["User,Action,Entity,Detail,Date,IP,Device", ...rows.map((a) => [a.user, a.action, a.entity, `"${a.detail}"`, a.date, a.ip, a.device].join(","))].join("\n")], { type: "text/csv" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "audit-log.csv"; a.click(); setTimeout(() => URL.revokeObjectURL(a.href), 3000); toast("Audit log exported"); }}><Ic n="download" size={15} />Export</button>
      </PageHead>
      <div className="panel overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="tbl">
            <thead><tr><th>Actor</th><th>Action</th><th className="hidden md:table-cell">Detail</th><th className="hidden lg:table-cell">IP address</th><th className="hidden xl:table-cell">Device</th><th>When</th></tr></thead>
            <tbody>
              {rows.map((a) => (
                <tr key={a.id}>
                  <td><span className="flex items-center gap-2.5"><Avatar first={a.user.split(" ")[0]} last={a.user.split(" ")[1] ?? "S"} hue={(a.user.length * 37) % 360} size={28} /><span><b className="block text-[12.5px] whitespace-nowrap">{a.user}</b><span className="block text-[10.5px] text-ink-400 uppercase font-bold">{a.role}</span></span></span></td>
                  ...
                  <td className="hidden md:table-cell text-[12.5px] max-w-[320px] truncate">{a.detail}</td>
                  <td className="hidden lg:table-cell font-mono text-[11.5px] text-ink-400 whitespace-nowrap">{a.ip}</td>
                  <td className="hidden xl:table-cell text-[12px] text-ink-400 whitespace-nowrap">{a.device}</td>
            ...
            <tbody>
              {db.backups.map((b) => (
                <tr key={b.id}>
                  ...
                  <td className="tnum font-semibold whitespace-nowrap">{b.size}</td>
                  <td><Chip tone="green"><Ic n="check" size={11} />completed</Chip></td>
                  <td className="text-right whitespace-nowrap">
                    <button className="btn-g btn-sm" onClick={() => {
                      const blob = new Blob([JSON.stringify({ type: "vitech-backup", date: b.date, school: db.school.name, records: { students: db.students.length, payments: db.payments.length, grades: db.grades.length } }, null, 2)], { type: "application/json" });
                      const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `vitech-backup-${b.date.slice(0, 10)}.json`; a.click();
                      setTimeout(() => URL.revokeObjectURL(a.href), 3000);
                      audit("DOWNLOAD_BACKUP", "Backup", `Snapshot ${b.date}`); toast("Snapshot downloaded", "info");
                    }}><Ic n="download" size={13} />Download</button>
  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-4">
        <div className="panel"><div className="panel-h"><h3 className="font-display font-bold text-[16px]">{tt("Student enrollment")}</h3><Chip tone="blue">admissions / month</Chip></div><div className="px-5 pb-5"><AreaChart key={`e${period}`} data={enroll.map((x) => x || 1)} labels={mk.map(monthLabel)} h={120} id="an1" /></div></div>
        <div className="panel"><div className="panel-h"><h3 className="font-display font-bold text-[16px]">{tt("Monthly revenue")}</h3><Chip tone="green">{cur}</Chip></div><div className="px-5 pb-5"><AreaChart key={`r${period}`} data={revBy} labels={mk.map(monthLabel)} h={120} color="#c98f1b" id="an2" /></div></div>
      </div>
...
export function PlatformPage() {
  ...
  return (
    <div>
          ...
          <div className="overflow-x-auto">
            <table className="tbl">
              ...
              <tbody>
                {db.tenants.map((t) => (
                  <tr key={t.id}>
                    <td><span className="flex items-center gap-3"><span className="w-9 h-9 rounded-lg bg-ink-950 dark:bg-cobalt-600 text-gold-400 flex items-center justify-center font-display font-bold text-[13px]">{t.name[0]}</span><span><b className="block text-[13px] whitespace-nowrap">{t.name}</b><span className="block text-[11px] text-ink-400">{t.city}</span></span></span></td>
                    ...
                    <td className="text-right">
                      <button className="btn-g btn-sm" onClick={() => { mutate((db) => { const x = db.tenants.find((y) => y.id === t.id)!; x.status = x.status === "suspended" ? "active" : "suspended"; }); audit("UPDATE_TENANT", "Platform", `${t.name} ${t.status === "suspended" ? "reactivated" : "suspended"}`); toast(`${t.name} ${t.status === "suspended" ? "reactivated" : "suspended"}`, "info"); }}>{t.status === "suspended" ? "Reactivate" : "Suspend"}</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <span className="hidden">{CURRENCY_MAP.USD.code}</span>
    </div>
  );
}
