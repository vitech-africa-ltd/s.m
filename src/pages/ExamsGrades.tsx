...
import { useApp, mutate, audit, notify, uid, daysAhead, fmtDate, gradeLetter, examAvg, classRanking, attStatus, lastSchoolDays, classOf, type Student, type Exam } from "../lib/data";
          ...
          return (
            <div key={e.id} className="panel p-5 hover:shadow-lift hover:-translate-y-0.5 transition-all duration-200">
              ...
              {e.status !== "scheduled" ? (
                ...
                <div className="rounded-lg bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-800 px-3.5 py-2.5 mt-4 text-[12px] font-bold text-amber-700 dark:text-amber-300 flex items-center gap-2"><Ic n="clock" size={14} />Begins in {Math.max(0, Math.round((new Date(e.date).getTime() - Date.now()) / 86400000))} days — reminders queued</div>
              )}
            </div>
          );
        })}
      ...
      <div className="grid lg:grid-cols-[1fr_290px] gap-4">
        ...
        <div className="space-y-4">
          <div className="panel p-5">
            <h3 className="font-display font-bold text-[15px] mb-3 flex items-center gap-2"><Ic n="award" size={16} className="text-gold-500" />Class ranking — {exam?.name.split("—")[0]}</h3>
            <div className="space-y-2">
              ...
              {ranking.map((r, i) => (
                <div key={r.s.id} className="flex items-center gap-2.5">
                  ...
                  <Avatar first={r.s.first} last={r.s.last} hue={r.s.hue} size={26} />
                ...
                {ranking.map((r, i) => {
                  ...
                  return (
                    <tr key={r.s.id}>
                      ...
                      <td><span className="flex items-center gap-2.5"><Avatar first={r.s.first} last={r.s.last} hue={r.s.hue} size={30} /><b className="text-[13.5px]">{r.s.first} {r.s.last}</b></span></td>
...
export function ReportCardDoc({ st, examId, tpl, comment, rank, of }: { st: Student; examId: string; tpl: typeof TEMPLATES[number]; comment: string; rank: number; of: number }) {
  const s = useApp();
  const db = s.db;
  const exam = db.exams.find((e) => e.id === examId);
  ...
  return (
    <div className="print-card mx-auto max-w-[760px] bg-white text-ink-900 border border-ink-200 rounded-lg overflow-hidden" style={{ fontFamily: "'Public Sans', sans-serif" }}>
      <div className="px-8 py-5 text-white flex items-center gap-4" style={{ background: tpl.head }}>
        <span className="w-12 h-12 rounded-xl flex items-center justify-center font-display font-bold text-2xl shrink-0" style={{ background: tpl.accent, color: "#fff" }}>{db.school.logoText[0]}</span>
        <div className="flex-1">
          ...
          <div className="text-[11px] opacity-75 mt-0.5">{db.school.address} · {db.school.phone} · {db.school.email}</div>
        </div>
      ...
      <div className="px-8 py-4 flex items-center gap-4 border-b border-ink-100">
        <Avatar first={st.first} last={st.last} hue={st.hue} size={54} />
